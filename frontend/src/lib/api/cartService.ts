...
class CartService {
		...
		if (typeof window !== "undefined") {
			...
			if (!savedUser) {
				...
				return {
					...
					data: this.convertGuestCartToCart({
						items: [],
						subtotal: 0,
						totalItems: 0,
					}),
	...
	private addToGuestCart(variantId: string, quantity: number): GuestCart {
		...
		if (existingItemIndex !== -1) {
			...
			guestCart.items[existingItemIndex].itemSubtotal =
				guestCart.items[existingItemIndex].variant.price *
				guestCart.items[existingItemIndex].quantity;
		} else {
		...
		guestCart.totalItems = guestCart.items.reduce(
	...
	async mergeGuestCartWithUserCart(): Promise<ApiResponse<Cart>> {
		...
		// Merge all items in parallel
		const results = await Promise.allSettled(
			guestCart.items.map((item) =>
				this.addCartItem(item.variantId, item.quantity),
			),
		);
		...
		if (failures.length > 0) {
			console.warn(
				`Failed to merge ${failures.length}/${guestCart.items.length} cart items:`,
				failures.map((f) => f.reason),
			);
		}

		// Only clear guest cart if at least one item merged successfully
		const hasSuccess = results.some((r) => r.status === "fulfilled");
		if (hasSuccess) {
			// If all succeeded, clear entire guest cart
			if (failures.length === 0) {
				this.clearGuestCart();
			} else {
				// If partial success, keep only failed items in guest cart
				const failedItems = guestCart.items.filter(
					(_, index) => results[index].status === "rejected",
				);
				const updatedGuestCart: GuestCart = {
					items: failedItems,
					subtotal: failedItems.reduce(
						(sum, item) => sum + item.itemSubtotal,
						0,
					),
					totalItems: failedItems.reduce((sum, item) => sum + item.quantity, 0),
				};
				this.saveGuestCart(updatedGuestCart);
			}
		}
	...
	updateGuestCartItemVariant(
		variantId: string,
		variantData: CartVariant,
	): void {
		const guestCart = this.getGuestCart();
		...
		if (itemIndex !== -1) {
			...
			guestCart.items[itemIndex].itemSubtotal =
				variantData.price * guestCart.items[itemIndex].quantity;
